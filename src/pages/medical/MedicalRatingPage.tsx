...
export default function MedicalRatingPage() {
  const navigate = useNavigate();
  const { id } = useParams();
  const { appointments, doctors } = useMedical();
  const app = appointments.find(a => a.id === id) || appointments[0];
  ...
  const handleSubmit = () => {
    if (rating === 0) return;
    navigate("/medical/success?type=rating");
  };
  ...
  return (
    <div className="min-h-screen bg-[#F5F7FA] pb-40">
      ...
      <header className="bg-white px-6 pt-12 pb-6 rounded-b-[40px] shadow-sm sticky top-0 z-30 flex items-center gap-4">
        ...
        <h1 className="text-xl font-black text-[#050505]">تقييم الطبيب</h1>
      </header>
      ...
      <main className="p-6 space-y-10">
        ...
        <section className="bg-white p-8 rounded-[48px] shadow-sm border border-gray-50 flex flex-col items-center text-center space-y-4">
          ...
          <div className="space-y-1">
            <div className="flex items-center justify-center gap-2">
              <h3 className="text-xl font-black text-[#050505]">{doc.name}</h3>
          ...
          <div className="bg-blue-50 px-4 py-1.5 rounded-full border border-blue-100">
            <p className="text-[10px] font-black text-[#1E90FF] uppercase tracking-widest">{app.date} - {app.time}</p>
          </div>
        </section>
        ...
        <section className="space-y-4">
          ...
          <textarea
            ...
            placeholder="شاركنا رأيك في جودة الرعاية الطبية، المواعيد، والتعامل..."
            className="w-full p-6 bg-white border border-gray-50 rounded-[32px] text-sm font-bold focus:ring-2 focus:ring-[#1E90FF] outline-none transition-all placeholder:text-gray-400 shadow-sm min-h-[150px] resize-none"
          />
        </section>

        {/* Photo Upload Placeholder */}
        <section className="space-y-4">
          <h4 className="text-sm font-black text-gray-400 uppercase tracking-widest px-2">إضافة صور (اختياري)</h4>
          <div className="flex gap-4">
            <button className="w-24 h-24 bg-white border-2 border-dashed border-gray-200 rounded-[28px] flex flex-col items-center justify-center text-gray-400 hover:border-[#1E90FF] hover:text-[#1E90FF] transition-all group">
