  ...
  if (cart.length === 0) {
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center p-8 text-center space-y-6">
        <div className="w-48 h-48 bg-blue-50 rounded-full flex items-center justify-center text-[#1877F2]">
          <ShoppingBag size={80} strokeWidth={1.5} />
        </div>
        <div className="space-y-2">
          <h2 className="text-2xl font-black text-[#050505]">سلتك فارغة</h2>
          <p className="text-gray-400 text-sm max-w-[250px] mx-auto">يبدو أنك لم تختر أي وجبات لذيذة بعد. ابدأ بالتسوق الآن!</p>
        </div>
        <button 
          onClick={() => navigate("/delivery")}
          className="px-10 py-4 bg-[#1877F2] text-white rounded-2xl font-black shadow-lg shadow-blue-100 active:scale-95 transition-all"
        >
          تصفح المطاعم
        </button>
      </div>
  ...
  return (
    <div className="min-h-screen bg-[#F0F2F5] pb-40">
      ...
      <header className="bg-white px-4 pt-12 pb-4 sticky top-0 z-30 shadow-sm flex items-center gap-4">
        ...
        <h1 className="text-xl font-black text-[#050505]">سلة الطلبات</h1>
      </header>
      ...
      <main className="p-4 space-y-6">
        ...
        <div className="bg-white rounded-[32px] p-6 shadow-sm border border-gray-100 space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm font-black text-[#050505]">{currentAddress}</p>
              </div>
            </div>
            <button className="text-xs font-black text-[#1877F2]">تغيير</button>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm font-black text-[#050505]">25 - 35 دقيقة</p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <h2 className="text-lg font-black text-[#050505] px-2">ملخص الطلب</h2>
          <AnimatePresence>
            {cart.map((item) => (
              <motion.div
                layout
                ...
                exit={{ opacity: 0, x: -20 }}
                key={item.id}
                className="bg-white rounded-[32px] p-4 shadow-sm border border-gray-100 flex gap-4"
              >
                ...
                <div className="flex-1 flex flex-col justify-between py-1">
                  <div className="flex justify-between items-start">
                    <h3 className="font-black text-[#050505] text-sm">{item.name}</h3>
                    <button 
                  ...
                  <div className="flex justify-between items-center">
                    <p className="font-black text-[#1877F2]">{item.price * item.quantity} ج.م</p>
                    <div className="flex items-center gap-3 bg-gray-50 rounded-xl px-2 py-1">
                      ...
                      <span className="text-xs font-black text-[#050505] w-4 text-center">{item.quantity}</span>
                      <button 
                        onClick={() => updateQuantity(item.id, 1)}
                        className="w-6 h-6 flex items-center justify-center text-[#1877F2]"
                      >
                        <Plus size={14} />
                      </button>
                    </div>
        ...
        {/* Promo Code */}
        <div className="bg-white rounded-[32px] p-4 shadow-sm border border-gray-100 flex items-center gap-3">
          ...
          />
          <button className="text-sm font-black text-[#1877F2] px-4">تطبيق</button>
        </div>
        ...
        <div className="bg-white rounded-[32px] p-6 shadow-sm border border-gray-100 space-y-4">
          <div className="flex justify-between text-sm">
            ...
            <span className="font-black text-[#050505]">{totalPrice} ج.م</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="font-bold text-gray-400">رسوم التوصيل</span>
            <span className="font-black text-[#050505]">{deliveryFee} ج.م</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="font-bold text-gray-400">رسوم الخدمة</span>
            <span className="font-black text-[#050505]">{serviceFee} ج.م</span>
          </div>
          <div className="h-px bg-gray-50" />
          <div className="flex justify-between text-lg">
            <span className="font-black text-[#050505]">الإجمالي</span>
            <span className="font-black text-[#1877F2]">{total} ج.م</span>
          </div>
        </div>
      </main>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white p-6 border-t border-gray-100 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] z-30">
        <button 
          ...
          className="w-full py-5 bg-[#1877F2] text-white rounded-[28px] font-black text-lg shadow-xl shadow-blue-100 flex items-center justify-between px-8 active:scale-95 transition-all"
        >
